(module opact MODULE_ADMIN
    ...
    (defun is-spent (nullifier:integer)
        (
            let*
            (
                (spent (length (select nullifierHashes (where 'value (= nullifier)))))
            )
            (= spent 1)
        )
    )
    ...
    (defun set-nullifierHash (key:string value:integer)
        (write nullifierHashes key { "value": value }))

    (defun verify-proof (proof:object{Proof})
        (at 'paired (plonk.verify proof)))
    ...
    (defun transact (args:object{Args} proof:object{Proof} extData:object{ExtData})
        (
            ...
            (
                ...
                (inputNullifier_0 (at 0 inputNullifiers))

                (isSpent_0 (is-spent inputNullifier_0))
            ...
            (enforce (= isSpent_0 false) "Input is already spent")
            ...
            ; actions
            (set-nullifierHash (int-to-str 10 inputNullifier_0) inputNullifier_0)
            ...
            (
                let*
                (
                    (leaf_0 (merkle.insert-leaf outputCommitment_0))
                    (leaf_1 (merkle.insert-leaf outputCommitment_1))

                    (leaf_0_index (at 'index leaf_0))
                    (leaf_1_index (at 'index leaf_1))
                )

                ; events
                (emit-event (new-commitment outputCommitment_0 leaf_0_index encryptedOutput_0))
                (emit-event (new-commitment outputCommitment_1 leaf_1_index encryptedOutput_1))            
            )
            (emit-event (new-nullifier inputNullifier_0))
            { 
