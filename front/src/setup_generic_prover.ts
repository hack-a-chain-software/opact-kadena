...
import {
  BarretenbergWasm,
  WorkerPool,
} from "@noir-lang/barretenberg/dest/wasm";
...
class StandardExampleProver {
  ...
  public async initCircuitDefinition(constraint_system: Uint8Array) {
    ...
    const constraint_system_ptr = await worker.call(
      "bbmalloc",
      constraint_system.length
    );
    await worker.transferToHeap(constraint_system, constraint_system_ptr);

    await worker.call(
      "standard_example__init_circuit_def",
      constraint_system_ptr
    );
  }
