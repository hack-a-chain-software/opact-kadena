...
.dp__input_valid {
  box-shadow: 0 0 var(--dp-border-radius)
    var(--dp-success-color);
...
@media only screen and (width <=600px) {
  .dp--preset-dates {
    ...
    overflow-x: auto;
    max-width: calc(
      var(--dp-menu-width) - var(--dp-action-row-padding) *
        2
    );
  }
}
...
.calendar-prev-leave-active {
  transition: all var(--dp-transition-timing-general)
    ease-out;
}
...
.calendar-next-leave-to {
  ...
  transform: translateX(
    calc(var(--dp-transition-length) * -1)
  );
}
...
.dp-slide-down-leave-active {
  transition: all var(--dp-animation-duration)
    var(--dp-transition-timing);
}
