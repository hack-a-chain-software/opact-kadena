...
const filterValidation = {
  ...
  search: {
    check: ({ sender, receiver, type }: any) => {
      return (
        sender.includes(search.value) ||
        receiver.includes(search.value) ||
        type.includes(search.value)
      )
    }
  }
}
...
const filtered = computed(() => {
  return receiptState.value.filter((receipt: any) => {
    return data.filters.every((filter: string) =>
      filterValidation[filter].check(receipt)
    )
  })
})
...
<template>
  ...
  >
    ...
    <div v-if="receiptState?.length > 0" class="w-full">
      <HistoryGroup
