...
export const useInvoice = () => {
  ...
  const pubkey = computed(
    () => data.address || params.value.pubkey
  )

  const amount = computed(
    () => data.amount || params.value.amount
  )

  const tokenId = computed(
    () => params.value.tokenId || data.token
  )

  const token = computed(() => {
    return (
      tokens.find(
        ({ id }: any) => id === Number(tokenId.value)
      ) || data.token
    )
  })
  ...
  const buttonIsDisabled = computed(
    () =>
      !pubkey.value ||
      !amount.value ||
      data.balance < Number(amount.value) ||
      amount.value === '0.0'
  )
