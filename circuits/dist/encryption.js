...
import { poseidonHash } from "./utils.js";
...
class Keypair {
    ...
    constructor(privkey = Wallet.createRandom().privateKey) {
        ...
        this.pubkey = poseidonHash([BigInt(this.privkey)]);
        this.encryptionKey = getEncryptionPublicKey(privkey.slice(2));
    }
    toString() {
        return "0x" + this.pubkey + Buffer.from(this.encryptionKey, 'base64').toString('hex');
    }
    /**
    ...
    static fromString(str) {
        ...
        return Object.assign(new Keypair(), {
            privkey: null,
            pubkey: toBigInt('0x' + str.slice(0, 64)),
     ...
     */
    encrypt(bytes) {
        return packEncryptedMessage(encrypt({
...
console.log({
    ...
    encryption: keypair.encryptionKey,
});
// module.exports = {
//   Keypair,
